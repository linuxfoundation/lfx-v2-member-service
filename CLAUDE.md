...
3. Fix the issues raised by that review and complete the repository's documentation-currency updates. Commit all resulting changes with `git commit -s -S`, then run `/member-service-pr-readiness` and `/member-service-preflight` against the clean, committed `HEAD`. If either check requires fixes, create another signed/DCO commit and rerun the affected deterministic checks. Do not run the local reviewers again.
