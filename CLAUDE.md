...
5. **When the reviews return:** roll every Critical finding and every reasonable Important finding into the next commit. The reviewers only report — fixes are always made by the parent session, never by a reviewer child. If any child fails, returns an empty report, states SHAs that do not match the pins, or leads with `INCOMPLETE`, resolve the cause and re-launch the whole three-child batch under the same pinned range.
...
3. **Full-branch sweep — if the branch has more than one commit, or on any branch when the final-commit optimization was used.** Pin `target_sha` = the branch head and `base_sha` = the branch's merge-base with `origin/main` (`git merge-base origin/main HEAD` after `git fetch origin`), then launch the same three skill-loading children (same batch shape, `model: opus`, `run_in_background: true`), each with prompt **`Load the skill <skill> and perform its review.\n\ntarget repo: lfx-v2-member-service\nbranch\ntarget_sha: <pinned target_sha>\nbase_sha: <pinned base_sha>\nreview exactly: git diff <base_sha> <target_sha> (the branch's diff against origin/main)\nThe pinned range is authoritative: derive the diff with git diff <base_sha> <target_sha> wherever the loaded skill's instructions name git show, HEAD, or origin/main...HEAD as the range (git show prints no diff hunks on a merge commit), and make per-file reads from the pinned revisions: for added or modified files, read git show <target_sha>:<path>; for deleted files, read git show <base_sha>:<path>; for renames, read both git show <base_sha>:<old-path> and git show <target_sha>:<new-path>. Never use the moving working-tree copy as evidence. Rule-surface and knowledge-base files are still read at current contents as the skill directs, and working-tree build/test/lint checks remain allowed under the loaded skill's own freshness conditions.\nReport findings only; never edit tracked files, stage, commit, push, or write GitHub state.\nStart your report with exactly these two lines, using the full 40-character SHAs:\ntarget_sha: <pinned target_sha>\nbase_sha: <pinned base_sha>\nIf the report is INCOMPLETE, keep INCOMPLETE as the first line and put those two SHA lines immediately after it.\n<final line — substituted per child exactly as in the post-commit step>`**. If any child fails, returns an empty report, states SHAs that do not match the pins, or leads with `INCOMPLETE`, resolve the cause and re-launch the whole three-child batch under the same pins. Address any new findings with one fix commit, then run ONE new full-branch trio on the updated branch; do not launch a post-commit trio for the fix.
