...
2. **If any returned review flags Critical or reasonable Important:** add a fix commit, launch the three reviewer subagents again on the new state, wait, and loop until clean or explicitly documented as a trade-off.
3. **Full-branch sweep — only if the branch has more than one commit.** Pin `target_sha` = the branch head and `base_sha` = the branch's merge-base with `origin/main` (`git merge-base origin/main HEAD` after `git fetch origin`), then launch the same three skill-loading children (same batch shape, `model: opus`, `run_in_background: true`), each with prompt **`Load the skill <skill> and perform its review.\n\ntarget repo: lfx-v2-member-service\nbranch\ntarget_sha: <pinned target_sha>\nbase_sha: <pinned base_sha>\nreview exactly: git diff <base_sha> <target_sha> (the branch's diff against origin/main)\nThe pinned range is authoritative: derive the diff with git diff <base_sha> <target_sha> wherever the loaded skill's instructions name git show, HEAD, or origin/main...HEAD as the range (git show prints no diff hunks on a merge commit), and make per-file reads of the changed files at the pinned revision with git show <target_sha>:<path> — never the working-tree copy of a changed file, since the tree may have moved past the pinned commit. Rule-surface and knowledge-base files are still read at current contents as the skill directs, and working-tree build/test/lint checks remain allowed under the loaded skill's own freshness conditions.\n<final line — substituted per child exactly as in the post-commit step>`**. Address any new findings, then re-run the sweep until clean.
