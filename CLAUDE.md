...
3. Fix the issues raised by that review, run the repository's documentation-currency checks, `/member-service-pr-readiness`, and `/member-service-preflight`, then create the signed/DCO commit. Do not run the local reviewers again for that commit.
