...
3. **Post-commit mode prompt for each child (exact, substituting the pinned SHAs, the skill name, and the range label):** `Load the skill <skill> and perform its review.\n\ntarget repo: lfx-v2-member-service\ntarget_sha: <pinned target_sha>\nbase_sha: <pinned base_sha>\nreview exactly: git diff <base_sha> <target_sha> (<range label>)\nThe pinned range is authoritative: derive the diff with git diff <base_sha> <target_sha> wherever the loaded skill's instructions name git show, HEAD, or origin/main...HEAD as the range (git show prints no diff hunks on a merge commit), and make per-file reads from the pinned revisions: for added or modified files, read git show <target_sha>:<path>; for deleted files, read git show <base_sha>:<path>; for renames, read both git show <base_sha>:<old-path> and git show <target_sha>:<new-path>. Never use the moving working-tree copy as evidence. Rule-surface and knowledge-base files are still read at current contents as the skill directs, and working-tree build/test/lint checks remain allowed under the loaded skill's own freshness conditions.\nReport findings only; never edit tracked files, stage, commit, push, or write GitHub state.\nStart your report with exactly these two lines, using the full 40-character SHAs:\ntarget_sha: <pinned target_sha>\nbase_sha: <pinned base_sha>\nIf the report is INCOMPLETE, keep INCOMPLETE as the first line and put those two SHA lines immediately after it.\n<final line — see below>` The `<range label>` is `the latest commit` for a normal post-commit batch, `the commits since the last review` for a coalesced batch (per the Batch invariant), or the merged branch name when a merge commit is reviewed anyway (per step 2's edge cases). The final line is substituted per child. For children 2 and 3 (the repo-owned skills) it is: `If <skill> is not in your skill registry, read <repo-root>/.claude/skills/<skill>/SKILL.md (locate the lfx-v2-member-service repo root first if you are not already inside it), follow it exactly as if it had been loaded, and state in your report that you loaded it from that path.` For child 1 it is: `If lfx-skills:lfx-general-code-review is not in your skill registry, do not review unguided: return a report whose first line is "INCOMPLETE — lfx-skills:lfx-general-code-review unavailable".` (Install the plugin per the note at the top of this file.) Append `extra: <focus>` on a new line only when there is a priority hint to add. Do NOT pass `branch` here. If this work cycle is launched from the LFX workspace parent, the `target repo:` line is required so all reviewers operate in this repo.
