...
> - For the post-commit review lifecycle, after every commit launch three generic background subagents in one parallel batch (all `model: opus`, `run_in_background: true`); each child explicitly loads exactly one review skill: `lfx-skills:lfx-general-code-review` for general code review, the repo-owned `member-service-code-reviewer` skill for repo-specific member-service conventions/contracts, and the repo-owned `member-service-learnings-reviewer` skill for empirical-pattern matching against `docs/reviews/knowledge-base/`. See the Work cycle section for the exact prompts.
> - **Local skills:**
...
2. **If any returned review flags Critical or reasonable Important:** add a fix commit, launch the three reviewer subagents again on the new state (or, if the fix responds to sweep findings, re-run the full-branch trio only per the final-commit optimization), wait, and loop until clean or explicitly documented as a trade-off.
