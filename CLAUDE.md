...
3. **Post-commit mode prompt for each child (exact, substituting the pinned SHAs and the skill name):** `Load the skill <skill> and perform its review.\n\ntarget repo: lfx-v2-member-service\ntarget_sha: <pinned target_sha>\nbase_sha: <pinned base_sha>\nreview exactly: git diff <base_sha> <target_sha> (the latest commit)` Append `extra: <focus>` on a new line only when there is a priority hint to add. Do NOT pass `branch` here. If this work cycle is launched from the LFX workspace parent, the `target repo:` line is required so all reviewers operate in this repo. The pinned range is authoritative: where a loaded skill's own instructions reference `HEAD` (or `origin/main...HEAD` in branch mode), the child applies them to the pinned `<base_sha> <target_sha>` range instead, so the review does not move with the branch. If a named skill is not present in the child's skill registry, the child reads the corresponding `.claude/skills/<skill>/SKILL.md` and follows it exactly as if it had been loaded.
