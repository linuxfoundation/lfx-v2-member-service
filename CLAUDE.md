...
> - For the post-commit review lifecycle, after every commit (except the final planned commit — see the Work cycle's final-commit optimization — and deferring/coalescing when a review batch is already running, per the Work cycle's Batch invariant) launch three generic background subagents in one parallel batch (all `model: opus`, `run_in_background: true`); each child explicitly loads exactly one review skill: `lfx-skills:lfx-general-code-review` for general code review, the repo-owned `member-service-code-reviewer` skill for repo-specific member-service conventions/contracts, and the repo-owned `member-service-learnings-reviewer` skill for empirical-pattern matching against `docs/reviews/knowledge-base/`. See the Work cycle section for the exact prompts.
> - **Local skills:**
...
>   - `member-service-code-reviewer` and `member-service-learnings-reviewer` are the repo-owned pre-PR review skills (loaded by the post-commit review subagents, not invoked by hand); `.claude/skills/local-code-review` and `.claude/skills/local-learnings-review` are their standard discovery aliases.
...
> **CRITICAL — while the branch is pre-PR, post-commit review is mandatory.** After every commit on the local branch — except the final planned commit under the Final-commit optimization below — launch **three generic background subagents in one parallel batch** (all `model: opus`, `run_in_background: true`) — then keep working while they run.
>
> **Batch invariant.** At most ONE review batch may be active at a time, and every batch is exactly THREE children. A batch is valid only when all three children return non-empty completed reports for the same pinned range. The post-commit cycle keeps a durable `review_coverage_boundary`: before the first batch, initialize it to that batch's pinned `base_sha`; advance it to the batch's pinned `target_sha` only after a valid complete three-child batch. A failed, empty, wrong-range, or `INCOMPLETE` child invalidates the whole batch and never advances the boundary. Resolve the cause, then re-launch all three children — never only one. If no commits accumulated, re-launch with the same pins; if commits accumulated, launch ONE coalesced batch with `base_sha` = the unchanged `review_coverage_boundary` and `target_sha` = current `HEAD`, labelled "the commits since the last review", so the earliest unreviewed commit cannot be skipped. You MUST drain the active trio before starting another batch. If a commit lands while a trio is still running, do NOT launch a second batch for it: keep working, and when the active trio drains, launch ONE coalesced batch from the durable boundary through current `HEAD` with the same label, so every interim commit is still covered. You must NEVER launch a full-branch trio while any post-commit trio is running, and — in the final-commit-optimization path — NEVER launch both trios for the commit that path skipped: launch no post-commit batch at all there, only the three-child full-branch batch. Each child explicitly loads exactly one review skill: `lfx-skills:lfx-general-code-review`, the repo-owned `member-service-code-reviewer`, or the repo-owned `member-service-learnings-reviewer`. All three children get the same target repo and the same immutable `target_sha`/`base_sha`/exact git diff range, pinned at launch. Each report must state the full target and base SHAs at its start as required by the exact prompt; the parent must compare those stated SHAs with the pins and reject the whole batch if either does not match. Before opening a PR, every running review must return clean (or remaining findings explicitly documented as trade-offs), the **full-branch sweep** must run clean if the branch has more than one commit or the final-commit optimization below was used (`branch` keyword), AND `/member-service-pr-readiness` must clear every Critical finding before `/member-service-preflight` runs.
>
> **Once the PR is open, do NOT launch the review subagents on iteration commits.** CodeRabbit + Copilot auto-trigger on every push and own the audit surface from that point. The skill-loading review subagents are pre-PR insurance only.
...
2. **Pin the range, then launch all three reviewer subagents in one parallel batch** — immediately if no review batch is active; if a trio is still running, defer and coalesce per the Batch invariant above. (If this is the final planned commit and you are moving immediately into pre-PR, skip this launch entirely — see the Final-commit optimization below.) Pin `target_sha` = the commit just made (`git rev-parse HEAD`) and `base_sha` = its first parent (`git rev-parse HEAD^`) — or, for a coalesced batch, `base_sha` = the durable `review_coverage_boundary` — **before** launching, so all three children review the identical immutable range even as the branch moves on. Before the first batch, initialize `review_coverage_boundary` to that batch's pinned `base_sha`; never initialize it from a later moving `HEAD`, and never advance it until the whole batch is valid. Edge cases: a root commit has no `HEAD^` — skip the post-commit review for it (root commits do not occur in this repo's workflow; if one ever must be reviewed, pin `base_sha` to the empty-tree hash `git hash-object -t tree /dev/null` so the template's `git diff` stays valid); for a merge commit skip the post-commit review — the full-branch sweep covers the merged-in work — or, if reviewing it anyway, label the range as the merged branch rather than "the latest commit", since `HEAD^` widens the diff to everything the merge brought in. Launch three generic subagents (e.g. `subagent_type: general-purpose`), every one with `model: opus` and `run_in_background: true`. Child 1 loads `lfx-skills:lfx-general-code-review`; child 2 loads the repo-owned `member-service-code-reviewer` skill; child 3 loads the repo-owned `member-service-learnings-reviewer` skill. Each child loads exactly its one skill — no other review skill.
3. **Post-commit mode prompt for each child (exact, substituting the pinned SHAs, the skill name, and the range label):** `Load the review instructions for <skill> from the authoritative source specified below and perform its review.\n\ntarget repo: lfx-v2-member-service\ntarget_sha: <pinned target_sha>\nbase_sha: <pinned base_sha>\nreview exactly: git diff <base_sha> <target_sha> (<range label>)\nThe pinned range is authoritative: derive the diff with git diff <base_sha> <target_sha> wherever the loaded instructions name git show, HEAD, or origin/main...HEAD as the range (git show prints no diff hunks on a merge commit), and make per-file reads from the pinned revisions: for added or modified files, read git show <target_sha>:<path>; for deleted files, read git show <base_sha>:<path>; for renames, read both git show <base_sha>:<old-path> and git show <target_sha>:<new-path>. Read every target-repo review-substrate file from <target_sha> with git show <target_sha>:<path>, including the repo-owned skill body, CLAUDE.md, rules, review checklists, architecture/contracts, and docs/reviews/knowledge-base/**; discover substrate paths with git ls-tree at <target_sha>, never from the moving filesystem. Never use a moving working-tree copy or fallback as review evidence. The central plugin's lfx-skills:lfx-general-code-review skill remains the installed central dependency because it is not target-repo substrate. Treat build/test/lint results as evidence only when they run against a checkout exactly at <target_sha>.\nReport findings only; never edit tracked files, stage, commit, push, or write GitHub state.\nStart your report with exactly these two lines, using the full 40-character SHAs:\ntarget_sha: <pinned target_sha>\nbase_sha: <pinned base_sha>\nIf the report is INCOMPLETE, keep INCOMPLETE as the first line and put those two SHA lines immediately after it.\n<final line — see below>` The `<range label>` is `the latest commit` for a normal post-commit batch, `the commits since the last review` for a coalesced batch (per the Batch invariant), or the merged branch name when a merge commit is reviewed anyway (per step 2's edge cases). The final line is substituted per child. For children 2 and 3 (the repo-owned skills) it is: `First read the authoritative repo-owned skill blob with git show <target_sha>:.claude/skills/<physical-skill>/SKILL.md, where <physical-skill> is member-service-code-reviewer or member-service-learnings-reviewer for this child. Use a named Skill-tool load only if you can show it is byte-equivalent to that target blob; otherwise follow the pinned blob exactly and state its target-SHA source in the report. Never read the moving working-tree skill file.` For child 1 it is: `lfx-skills:lfx-general-code-review is the installed central dependency, not target-repo substrate. Load it from the skill registry. If it is unavailable, do not review unguided: return a report whose first line is "INCOMPLETE — lfx-skills:lfx-general-code-review unavailable".` (Install the plugin per the note at the top of this file.) Append `extra: <focus>` on a new line only when there is a priority hint to add. Do NOT pass `branch` here. If this work cycle is launched from the LFX workspace parent, the `target repo:` line is required so all reviewers operate in this repo.
4. **Keep working.** Start the next commit while the reviewers run. Do not block on them — commits never wait; only batch launches serialize (a commit made while the trio is still running gets its review via the coalesced batch of the Batch invariant).
5. **When the reviews return:** roll every Critical finding and every reasonable Important finding into the next commit. The reviewers only report — fixes are always made by the parent session, never by a reviewer child. Advance `review_coverage_boundary` to the pinned `target_sha` only when all three reports make the batch valid. If any child fails, returns an empty report, states SHAs that do not match the pins, or leads with `INCOMPLETE`, leave the boundary unchanged, resolve the cause, and re-launch the whole three-child batch with the same pins when `HEAD` is unchanged or coalesced from that boundary through current `HEAD` when commits accumulated.

**Final-commit optimization.** Normal development commits always launch the post-commit trio above. When the parent knows the commit just made is the final planned commit and is moving immediately into pre-PR, skip that commit's post-commit trio; after draining earlier reviews, run only the full-branch trio (Pre-PR step 3) — in that case the sweep is mandatory even for a single-commit branch, since it is what covers the final commit. If development resumes with further planned commits before the sweep begins, return to normal per-commit review.

**Sweep-fix optimization.** Once the full-branch sweep phase has begun, every sweep-driven, readiness-driven, or preflight-driven fix commit skips the post-commit trio and goes directly to ONE new full-branch trio. Never run both a per-commit trio and a branch trio for such a fix.
...
2. **Before the full-branch sweep begins, if any returned post-commit review flags Critical or reasonable Important:** add a fix commit, launch the three reviewer subagents again on the new state, wait, and loop until clean or explicitly documented as a trade-off.
3. **Full-branch sweep — if the branch has more than one commit, or on any branch when the final-commit optimization was used.** Immediately before EVERY sweep launch or re-launch, independently run `git fetch origin`, then pin `base_sha=$(git merge-base origin/main HEAD)` and `target_sha=$(git rev-parse HEAD)`. A sweep is always cumulative from the freshly fetched `origin/main` merge-base through the freshly pinned branch head; never use the post-commit `review_coverage_boundary` as a sweep base. Then launch the same three skill-loading children (same batch shape, `model: opus`, `run_in_background: true`), each with prompt **`Load the review instructions for <skill> from the authoritative source specified below and perform its review.\n\ntarget repo: lfx-v2-member-service\nbranch\ntarget_sha: <pinned target_sha>\nbase_sha: <pinned base_sha>\nreview exactly: git diff <base_sha> <target_sha> (the branch's diff against origin/main)\nThe pinned range is authoritative: derive the diff with git diff <base_sha> <target_sha> wherever the loaded instructions name git show, HEAD, or origin/main...HEAD as the range (git show prints no diff hunks on a merge commit), and make per-file reads from the pinned revisions: for added or modified files, read git show <target_sha>:<path>; for deleted files, read git show <base_sha>:<path>; for renames, read both git show <base_sha>:<old-path> and git show <target_sha>:<new-path>. Read every target-repo review-substrate file from <target_sha> with git show <target_sha>:<path>, including the repo-owned skill body, CLAUDE.md, rules, review checklists, architecture/contracts, and docs/reviews/knowledge-base/**; discover substrate paths with git ls-tree at <target_sha>, never from the moving filesystem. Never use a moving working-tree copy or fallback as review evidence. The central plugin's lfx-skills:lfx-general-code-review skill remains the installed central dependency because it is not target-repo substrate. Treat build/test/lint results as evidence only when they run against a checkout exactly at <target_sha>.\nReport findings only; never edit tracked files, stage, commit, push, or write GitHub state.\nStart your report with exactly these two lines, using the full 40-character SHAs:\ntarget_sha: <pinned target_sha>\nbase_sha: <pinned base_sha>\nIf the report is INCOMPLETE, keep INCOMPLETE as the first line and put those two SHA lines immediately after it.\n<final line — substituted per child exactly as in the post-commit step>`**. If any child fails, returns an empty report, states SHAs that do not match the pins, or leads with `INCOMPLETE`, resolve the cause, restart this step from `git fetch origin`, re-pin both SHAs independently, and re-launch the whole three-child batch. Address any new findings with one fix commit, then restart this step from fetch and run ONE new full-branch trio on the updated branch; do not launch a post-commit trio for the fix.
4. **Run `/member-service-pr-readiness`** for branch and commit shape only. If readiness requires a fix commit, return directly to step 3 under the Sweep-fix optimization.
5. **Run `/member-service-preflight`** for mechanical Go validation and PR summary. If preflight requires a fix commit, return directly to step 3 under the Sweep-fix optimization.
