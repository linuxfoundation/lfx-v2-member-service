...
>
> **Batch invariant.** At most ONE review batch may be active at a time, and every batch is exactly THREE children (re-launching a single child that returned `INCOMPLETE` is a repair of the existing batch, not a new batch). You MUST drain the active trio before starting another batch. If a commit lands while a trio is still running, do NOT launch a second batch for it: keep working, and when the active trio drains, launch ONE coalesced batch pinned to `base_sha` = the last-reviewed commit and `target_sha` = current `HEAD` — labelled "the commits since the last review" instead of "the latest commit" — so every interim commit is still covered. You must NEVER launch a full-branch trio while any post-commit trio is running, and — in the final-commit-optimization path — NEVER launch both trios for the commit that path skipped: launch no post-commit batch at all there, only the three-child full-branch batch. (Outside that path, a pre-PR fix commit legitimately gets its per-commit trio and is later also covered by the mandatory sweep — that is not a violation.) Each child explicitly loads exactly one review skill: `lfx-skills:lfx-general-code-review`, the repo-owned `member-service-code-reviewer`, or the repo-owned `member-service-learnings-reviewer`. All three children get the same target repo and the same immutable `target_sha`/`base_sha`/exact git diff range, pinned at launch. Before opening a PR, every running review must return clean (or remaining findings explicitly documented as trade-offs), the **full-branch sweep** must run clean if the branch has more than one commit or the final-commit optimization below was used (`branch` keyword), AND `/member-service-pr-readiness` must clear every Critical finding before `/member-service-preflight` runs.
>
...
5. **When the reviews return:** roll every Critical finding and every reasonable Important finding into the next commit. The reviewers only report — fixes are always made by the parent session, never by a reviewer child. A report leading with `INCOMPLETE` does not count as clean; resolve the cause and re-launch that review.

**Final-commit optimization.** Normal development commits always launch the post-commit trio above. When the parent knows the commit just made is the final planned commit and is moving immediately into pre-PR, skip that commit's post-commit trio; after draining earlier reviews, run only the full-branch trio (Pre-PR step 3) — in that case the sweep is mandatory even for a single-commit branch, since it is what covers the final commit. If sweep findings require a fix commit, do not also run a per-commit trio — re-run the full-branch trio instead. If development resumes with further planned commits, return to normal per-commit review.
