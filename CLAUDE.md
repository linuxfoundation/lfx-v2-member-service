...
2. **Pin the range, then immediately launch all three reviewer subagents in one parallel batch.** Pin `target_sha` = the commit just made (`git rev-parse HEAD`) and `base_sha` = its first parent (`git rev-parse HEAD^`) **before** launching, so all three children review the identical immutable range even as the branch moves on. Launch three generic subagents (e.g. `subagent_type: general-purpose`), every one with `model: opus` and `run_in_background: true`. Child 1 loads `lfx-skills:lfx-general-code-review`; child 2 loads the repo-owned `member-service-code-reviewer` skill; child 3 loads the repo-owned `member-service-learnings-reviewer` skill. Each child loads exactly its one skill — no other review skill.
3. **Post-commit mode prompt for each child (exact, substituting the pinned SHAs and the skill name):** `Load the skill <skill> and perform its review.\n\ntarget repo: lfx-v2-member-service\ntarget_sha: <pinned target_sha>\nbase_sha: <pinned base_sha>\nreview exactly: git diff <base_sha> <target_sha> (the latest commit)\nThe pinned range is authoritative: where the loaded skill's instructions reference HEAD or origin/main...HEAD, apply them to the pinned range instead.\nIf the named repo-owned skill is not in your skill registry, read .claude/skills/<skill>/SKILL.md, follow it exactly as if it had been loaded, and state in your report that you loaded it from that path.` Append `extra: <focus>` on a new line only when there is a priority hint to add. Do NOT pass `branch` here. If this work cycle is launched from the LFX workspace parent, the `target repo:` line is required so all reviewers operate in this repo. The registry fallback applies only to the two repo-owned skills (children 2 and 3); if child 1's `lfx-skills:lfx-general-code-review` plugin skill is unavailable, that child returns a report leading with `INCOMPLETE` rather than reviewing unguided (install the plugin per the note at the top of this file).
