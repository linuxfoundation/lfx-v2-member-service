...
>
> **Carve-out: settling an unrelated stale pair.** `DELETE /project_memberships/{membership_uid}/key_contacts/{uid}` can additionally revoke a *different* `{membership, username}` pair before the Salesforce delete, when `key-contact-grants` shows the deleting record's own entry pointing at a stale pair left over from an earlier failed reparent/rename. This does not violate the property above: it is not this record's own cleanup (that still runs only after the delete, per the rule), the sibling scan for that stale pair already excludes the record being deleted by UID regardless of ordering, and the settlement must block the delete so a failure preserves the only remaining retry address (this record's index entry) for the client's retry: deleting the record first would leave that stale pair with no address at all. See [fga-contract.md](./fga-contract.md)'s sibling-aware revoke callout.
...
| `slug`                | string (optional)   | Lowercase URL slug derived from `name` by `model.Slugify` (spec 050 DR-007: accent-fold, lowercase, `[^a-z0-9]+`→`-`, trim, word-boundary cut at 50) — the organization's identity in Org Lens addresses (`/org/{slug}/…`, lfx-self-serve#2570). Omitted when the name yields no usable slug (the UI then addresses the org by `uid`). Not a Salesforce field; not stored; changes when the name changes. |
...
| `project_uid`           | string    | Association primary key — a UUID generated by member-service; stable for indexing and is the delete key |
| `project_slug`          | string    | Caller-owned project identifier supplied on add (e.g. an Org Lens project slug) |
| `project_name`          | string (optional) | Human-readable project name supplied on add; indexed in `fulltext`/`sort_name` for search |
...
| Tag Format                          | Example                                | Purpose                                            |
|-------------------------------------|----------------------------------------|----------------------------------------------------|
| `{workspaceUID}/{projectUID}`       | `550e8400-.../a1b2c3d4-...`            | Direct lookup by association (composite key); `{projectUID}` is the generated association UUID |
...
> `project_slug` is always present (caller-supplied); `project_name` is optional. When the caller omits `project_name`, `sort_name` and the name portion of `fulltext` are empty for that association, but slug keyword search still works. Lookup by the generated UUID uses the `project_uid:` tag, and lookup by caller slug uses the `project_slug:` tag.
