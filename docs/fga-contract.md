...
| CDC `Project_Role__ChangeEvent` | `project_membership` | `lfx.fga-sync.member_put` (+ `lfx.fga-sync.member_remove`) | LFID resolved via `UserReader.UsernameByEmail`; skipped when email has no LFID. A `member_remove` for the previously recorded `{membership_uid, username}` follows the put when the grant target changed (Salesforce-side reparent, or a changed LFID). On a *definitive* miss (email no longer resolves to any registered account) a `member_remove` also revokes any grant still recorded in `key-contact-grants` for this contact; a transport-level lookup failure never triggers this — see the CDC upsert path callout above |
