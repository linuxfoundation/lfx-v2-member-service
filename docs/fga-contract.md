...
**A purge does not leave zero tuples.** fga-sync preserves configured team-subject grants on the non-global `auditor` relation, and no service code path can reap them. They confer access to an object that no longer resolves, so they are inert, but any verification that asserts an empty tuple set for a deleted object will report a correct implementation as broken. Removing them requires a one-off script.
