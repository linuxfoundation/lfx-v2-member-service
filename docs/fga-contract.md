...
| Deactivate key contact (`Status` → `Inactive`, any upsert path) | `project_membership` | `lfx.fga-sync.member_remove` | Revokes the recorded grant, or the record's own resolved pair on a `key-contact-grants` miss. Skipped when a live sibling record still justifies the tuple (only the record's index entry is cleared), and skipped entirely when the sibling scan fails. See the Status gate callout |
