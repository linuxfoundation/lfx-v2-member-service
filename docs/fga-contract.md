...
>
> **Sibling-aware revoke:** the FGA tuple is keyed on `(membership, username)` while grants are per `Project_Role__c` record, so several records can back one tuple. Before an Inactive revoke, the service fetches the membership's current key contacts from Salesforce and looks for a live sibling: a different record, non-Inactive, with the same email. If one exists, the tuple stays and only the deactivated record's own `key-contact-grants` entry is cleared. If the sibling scan itself fails, the revoke is skipped entirely: a stale grant that a later pass can clean up is preferable to wrongly cutting off a live user. The same protection guards the email-change path: the old pair's `member_remove` is skipped when a live sibling still holds the old email.
...
| Update key contact (username change) | `project_membership` | `lfx.fga-sync.member_put` + `lfx.fga-sync.member_remove` | Grants the new username before revoking the old one, so no window leaves the contact without access. Skipped entirely when the username resolves unchanged. On an email change, the old pair's `member_remove` is skipped when a live sibling record still holds the old email |
...
| CDC `Project_Role__ChangeEvent` | `project_membership` | `lfx.fga-sync.member_put` (+ `lfx.fga-sync.member_remove`) | LFID resolved via `UserReader.UsernameByEmail`; skipped when email has no LFID. A `member_remove` for the previously recorded `{membership_uid, username}` follows the put when the grant target changed (Salesforce-side reparent, or a changed LFID). On a *definitive* miss (email no longer resolves to any registered account) a `member_remove` also revokes any grant still recorded in `key-contact-grants` for this contact; a transport-level lookup failure never triggers this — see the CDC upsert path callout above. An Inactive record publishes no put and takes the sibling-aware revoke path instead (see the Status gate callout) |
