...
`UsernameByEmail` distinguishes a definitive "no registered account" miss from a transport-level failure (auth-service unreachable). A definitive miss leaves `Username` empty (so `PublishKeyContactFGA` publishes nothing this pass) and revokes any grant still recorded for the contact in the `key-contact-grants` bucket — the email was renamed or deregistered since the last successful grant, so a stale FGA tuple would otherwise survive indefinitely. That revoke is sibling-checked through the shared choke point with a live per-membership read (not the page's batched prefetch, which may not cover the recorded pair's membership). A transport-level failure is not evidence the email is unregistered: it is logged and swallowed, `Username` stays empty, and any existing grant is left untouched for a later pass to retry.

An assembled record whose `Status` is `Inactive` takes the sibling-aware revoke path instead of publishing a put (see the sibling-aware revoke callout in [fga-contract.md](./fga-contract.md); a sibling justifies the pair by carrying the same email or an email resolving to the same LFID). The sibling check is batched here: each SOQL page or targeted batch issues at most one `FetchKeyContactsByAssetSFIDs` call covering the memberships of every contact on the page (Active contacts' supersede revokes consult it too), instead of one Salesforce query per contact. A membership outside the prefetch (a reparented old pair) falls back to a live per-membership read rather than reading as falsely sibling-free. When the prefetch fails, every affected revoke in the page is skipped (fail safe). Because a cold-index revoke targets the record's own resolved pair, a `key_contact` reindex also withdraws tuples for Inactive records granted before the bucket existed.

Each published grant is recorded in the `key-contact-grants` KV bucket, which is what lets a later CDC delete address the revoke. A `key_contact` reindex is therefore also a way to populate that bucket for contacts granted before it existed. Over a cold bucket, an Active record's run emits `member_put` only: there is no recorded grant to compare against, so no supersede `member_remove` traffic is produced. For an Active record, or one already deleted in Salesforce, it does **not** repair a tuple that was already orphaned before the bucket existed, those grants were never recorded, so nothing knows they need revoking. An Inactive record is the exception: as noted above, a cold index still revokes its own resolved pair, so a reindex does repair that record's own pre-index orphan even with no recorded grant to compare against. What stays unrepairable by a cold run is a *different* contact's orphaned pair (an Active sibling's, or one from a record already deleted) that this record's own revoke never touches.
