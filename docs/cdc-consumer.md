...
| Delete | Invalidate cache → `PublishKeyContactIndexer` (`deleted`, stub) → look up `key-contact-grants` → sibling-aware choke point for the recorded `{membership_uid, username}` pair (the record is already gone in Salesforce, so justification runs by LFID resolution alone) → when unjustified, `member_remove` targeting the recorded `project_membership:{membership_uid}` → **Flush** to confirm broker delivery → delete the index entry only once flush succeeds; when a live sibling record still justifies the pair, the tuple stays and the entry is deleted without publishing. A transient index read failure is retried (bounded, `maxGrantIndexReadAttempts`) before falling back, since the deleted contact has no other chance to retry once this handler returns. An inconclusive sibling scan or a failed flush leaves the entry in place and returns `errKeyContactRevokeIncomplete`, which holds the replay cursor (like `errPurgeUnrecorded`) so the event is redelivered and the revoke retried: the record is gone, so no later upsert or reindex would ever retry it otherwise. On an index miss (retries exhausted, or a contact granted before this bucket existed) it falls back to the contact's own SFID with an empty username, which fga-sync rejects — logged with `fga_revoke_failed_dangling_tuple=true`. |
