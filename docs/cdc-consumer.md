...
3. **Upserts (batched):**
   ...
   - **Publish** indexer + FGA messages for each present record.
...
| Change | Actions |
|---|---|
| Upsert | Invalidate cache → fetch memberships → resolve `project_uid` → `PublishProjectMembershipIndexer` (`created`/`updated`) + `PublishProjectMembershipFGA` (`b2b_org` + `project` refs; `key_contact` excluded). |
...
| Change | Actions |
|---|---|
| Upsert | Invalidate cache → fetch contacts → per contact (`processKeyContact`): resolve LFID via `UserReader.UsernameByEmail` when username is empty → resolve `project_uid` → `PublishKeyContactIndexer` + `PublishKeyContactFGA` (`member_put` only when an LFID resolved) → **silently** provision org-dashboard access for registered contacts (`AddPrincipal` with `SuppressNotification: true`). |
