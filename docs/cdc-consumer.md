...
CDCConsumer.Run  (internal/service/cdc_consumer.go)
  • one event at a time; commit replay cursor after each
    (except after an authorization change needs redelivery — see Held replay cursor)
...
| `GAP_UNDELETE` | upsert **+ grant restore** | Same restoration contract as `UNDELETE`; exact `isRestore` matching keeps unrelated `GAP_*` events on the ordinary upsert path. |
| `DELETE` | delete | Publishes a delete indexer event; no re-fetch. |
| `GAP_DELETE` | delete | Delete during a CDC overflow gap. |
| `GAP_OVERFLOW` / other `GAP_*` | upsert | Granular delivery was interrupted — re-fetch as an ordinary upsert and log a WARN. |
...
2. Each event is fully handled, then the cursor is `Save`d. Ordinary handler errors still advance so a repairable failure does not block the stream; incomplete delete/restore authorization changes hold the cursor because `/admin/reindex` cannot rebuild those relations.
...
3. **Upserts (batched):**
   - **Quota guard** — skip the re-fetch if the Salesforce REST API quota is near-exhausted (see [Quota guard](#quota-guard)). **Asset and Project_Role__c:** cache invalidation runs *before* the quota guard, so a stale record is dropped even when the re-fetch is skipped; the next read then costs one Salesforce call instead of serving stale data. Account invalidates after the guard.
   - **(Account only)** capture old record state for the reparenting diff before eviction.
   - **Invalidate** the sObject cache entry for each ID. **Asset only:** additionally evict the soft-TTL `membership-cache` entry (`DeleteMembership`) that `GetMemberTiers` serves from. **Project_Role__c only:** additionally evict the grouped `key-contacts.{membership_uid}` soft-TTL entry (`DeleteKeyContactsForMembership`) that the member-tiers eligibility revalidation reads: pre-guard via the `key-contact-grants` index (covers quota-skipped batches for granted contacts) and again post-fetch from each returned contact's `MembershipUID` (covers contacts not yet in the index). The evictor is optional (nil in mock mode), so these evictions are best-effort freshness, not a correctness guarantee.
...
| Delete | Invalidate cache → `PublishB2BOrgIndexer` (`deleted`, stub org) → `delete_access` for the UID, which withdraws the org's tuples (see [LFXV2-3034](https://linuxfoundation.atlassian.net/browse/LFXV2-3034)). Published on the **genuine-delete path only** — see the absent-path row below → **Flush** to confirm broker delivery, since `Access` alone returns success for a purge the broker never received. On a publish failure **or an unconfirmed flush**, the error propagates to `dispatchEntity` (which logs and moves on to the next ID — the batch is unaffected) and the UID is durably recorded under `ReindexTypeB2BOrgDeleteAccess` in the CDC repair KV bucket for manual recovery; `/admin/reindex` cannot auto-repair a dropped purge. `team:`-subject tuples survive regardless: fga-sync never deletes one, so the configured LF-team reader grants (staff and contractor) remain on a dead object. They are inert, but an audit asserting zero remaining tuples will wrongly report this as broken. |
...
| Change | Actions |
|---|---|
| Upsert | Invalidate sObject cache + evict grouped `key-contacts.{membership_uid}` soft-TTL entry via the grant index (both **before** the quota guard) → fetch contacts → evict the grouped entry again per returned contact's `MembershipUID` (deduped; covers contacts absent from the grant index) → per contact (`processKeyContact`): resolve LFID via `UserReader.UsernameByEmail` when username is empty → resolve `project_uid` → on success: `PublishKeyContactIndexer` + `PublishKeyContactFGA` (`member_put` only when an LFID resolved) + **silently** provision org-dashboard access (`AddPrincipal` with `SuppressNotification: true`). On resolver failure: skip indexer + provisioning, log ERROR, **OpenFGA only** (when LFID resolved). A published `member_put` is recorded in `key-contact-grants`; when the recorded `{membership_uid, username}` differs from the new one (Salesforce-side reparent, or a changed LFID) the superseded pair rides along as a `PendingRevoke` marker in the **same** CAS write that commits the replacement, then the superseded pair goes through the sibling-aware choke point: when a live sibling record still justifies it, the marker is cleared without publishing; otherwise a `member_remove` is published and flushed. The marker is cleared only once that flush confirms delivery — an inconclusive sibling scan, a lost publish, or an unconfirmed flush leaves it in place (`fga_revoke_failed_dangling_tuple=true`) rather than losing the superseded pair's address the instant the replacement commits. `UsernameByEmail` distinguishes a definitive "no registered account" miss from a transport-level failure: a definitive miss revokes any grant still recorded for the contact in `key-contact-grants` (the account was renamed or deregistered since the last successful grant); a transport-level failure is not treated as evidence the email is unregistered and leaves any existing grant untouched. A contact whose `Status` is `Inactive` (case-insensitive) never publishes a put: it revokes its recorded grant instead, or its own resolved pair when `key-contact-grants` has no usable entry. That revoke is sibling-aware: when another non-Inactive record on the same membership justifies the pair (same email, or an email resolving to the same LFID), the shared tuple stays and only the record's own index entry is cleared; an inconclusive sibling scan skips the revoke entirely (fail safe). Unlike an ordinary best-effort grant-recording failure elsewhere on this path, a failed or inconclusive revoke for an Inactive contact, or for a definitive email miss, holds the replay cursor (`errKeyContactRevokeIncomplete`) rather than logging and moving on: see [In-process authorization retry](#in-process-authorization-retry) below. The sibling data for a whole upsert page comes from one batched, membership-grouped Salesforce read (`batchedSiblingLister`) covering every membership on the page (supersede revokes consult it for Active contacts too) instead of a per-contact query; a membership outside the prefetch (a reparented old pair) and the definitive-miss revoke fall back to a live per-membership read, so an un-prefetched membership never reads as falsely sibling-free. A record deactivating with a `PendingRevoke` marker still on its entry first drains that superseded pair through the same choke point, clearing the marker only on a confirmed or justified outcome. See the sibling-aware revoke callout in [fga-contract.md](./fga-contract.md). |
| Delete | Invalidate sObject cache → `PublishKeyContactIndexer` (`deleted`, stub) → look up `key-contact-grants`; when a grant is recorded, evict the grouped `key-contacts.{membership_uid}` soft-TTL entry so the deleted contact stops passing the member-tiers revalidation → sibling-aware choke point for the recorded `{membership_uid, username}` pair (the record is already gone in Salesforce, so justification runs by LFID resolution alone) → when unjustified, `member_remove` targeting the recorded `project_membership:{membership_uid}` → **Flush** to confirm broker delivery → delete the index entry only once flush succeeds; when a live sibling record still justifies the pair, the tuple stays and the entry is deleted without publishing. A transient index read failure is retried (bounded, `maxGrantIndexReadAttempts`) before giving up, since the deleted contact has no other chance to retry once this handler returns. An inconclusive sibling scan, a failed flush, or exhausted read retries all leave the entry in place and return `errKeyContactRevokeIncomplete`, which holds the replay cursor (like `errPurgeUnrecorded`) so the event is redelivered and the revoke retried: the record is gone, so no later upsert or reindex would ever retry it otherwise. Only a genuine miss, a contact with no recorded entry at all (pre-index contact, or a grant never recorded), falls back to the contact's own SFID with an empty username, which fga-sync rejects, logged with `fga_revoke_failed_dangling_tuple=true`. |
...
`{membership_uid, username}` tuple. A deactivation (`Status` → `Inactive`) that
finds the marker does retry it through the sibling-aware choke point, since it
is the last scheduled visit to the entry; the marker survives any inconclusive
or failed attempt.
...
#### Quota-gated key_contact delete recheck

The quota guard above (batch skip → `cdc-repair` queue → cursor still advances) covers upsert re-fetches only. The `Project_Role__c` delete path's sibling recheck is gated separately and behaves differently on purpose: a delete's revoke must be provably safe (it never gets another chance once the record is gone), so a skip there cannot be treated as "repairable later" the way an upsert skip is.

Before prefetching sibling data for a batch of `Project_Role__c` deletes, `quotaGuardExceeded` is checked once for the whole batch. If quota is exceeded, the prefetch is replaced with a lister that always reports the scan as **quota-skipped**, which reads as inconclusive to the sibling-aware choke point, never as certainly sibling-free, so every unsettled delete in the batch returns `errKeyContactRevokeIncomplete` and **holds the replay cursor** via [In-process authorization retry](#in-process-authorization-retry) above, instead of being queued to `cdc-repair` and advancing. The same quota gate also wraps the live per-record recheck lister (`quotaGuardedLister`) used elsewhere in the delete path, for the same reason.

This means sustained quota pressure stalls the whole CDC channel on any pending key_contact delete, unlike an upsert skip, which never blocks the stream. The retry backoff (100 ms – 30 s) keeps this from hot-looping, and the stall self-clears once quota headroom returns (passively, or via the same active refresh described above).
...
- **If the marker write also fails, the same event is retried in-process** before another event is consumed — see [In-process authorization retry](#in-process-authorization-retry) below. That is the case where a purge has neither been delivered nor recorded, and therefore has no recovery route but retry.
- **Manual recovery today** (no `/admin/reindex` support): inspect the `cdc-repair` bucket directly for keys matching `pending.b2b_org_delete_access.*` / `pending.project_membership_delete_access.*` (e.g. via `nats kv` against the bucket). A marker can outlive a later `UNDELETE`, so **never replay it blindly**. Run the check as the same Salesforce integration user as the service, map `b2b_org_delete_access` to `Account` (`001…` ID) and `project_membership_delete_access` to `Asset` (`02i…` ID), and reject a marker whose prefix does not match its type. Immediately before handling each marker, use Salesforce `queryAll` (not ordinary `query`, which hides soft-deleted rows) to read `Id, IsDeleted`. Only an explicit row with `IsDeleted=true` authorizes `delete_access`; publish it, flush it, then remove the marker. An explicit `IsDeleted=false` means the object is live, so remove the stale marker without publishing. Zero rows, a prefix/type mismatch, insufficient visibility, or any source/flush error is inconclusive: fail closed by leaving the marker and publishing nothing.

  A check cannot make the check-to-publish interval atomic with Salesforce. Query the same record again after a confirmed purge and remove the marker only if it still reports `IsDeleted=true`. If the post-check reports live or is inconclusive, leave the marker and alert: there is no single restore command, and an ordinary CDC `UPDATE` is insufficient. For an Account, targeted-reindex the `b2b_org` and each current child (hierarchy/team references), then resubmit the current org settings through the normal settings write path with explicit accepted writer and auditor lists. For an Asset, targeted-reindex the `project_membership`, then targeted-reindex every current `Project_Role__c` key contact so each `member_put` is reissued. Confirm FGA convergence before removing the marker.

#### In-process authorization retry

Commit-after-process advances the replay cursor after ordinary handler errors, but not past an authorization change with no other complete repair route:

- A genuine-delete's `delete_access` was neither confirmed delivered nor recorded in `cdc-repair`.
- An `UNDELETE` or `GAP_UNDELETE` could not fetch a qualifying restored record, read authoritative grant state, resolve a non-definitive LFID, record a key-contact grant address, publish a grant, or confirm broker receipt with `Flush`.
- A `Project_Role__c` upsert whose record is `Inactive` (or a definitive LFID miss) could not confirm its `member_remove` revoke, wrapped as `errKeyContactRevokeIncomplete`, unlike an ordinary grant-recording failure on the active path, which stays best-effort logged (see the Status gate callout in [fga-contract.md](./fga-contract.md)).
- A `Project_Role__c` delete (direct, or reached through the absent-from-SOQL path) could not confirm its recorded grant's revoke, a `PendingRevoke` marker's drain, or a durable-address transfer to a justifying sibling, also wrapped as `errKeyContactRevokeIncomplete`, including when the delete path's own sibling-recheck quota gate skips the fetch (see [Quota-gated key_contact delete recheck](#quota-gated-key_contact-delete-recheck) below).

`Run` retries the incomplete event in-process and does not read a later event or save a cursor until the retry succeeds. This protects an empty replay store while the process remains alive; cancellation still exits without saving, because no predecessor cursor exists to persist. Retries use exponential backoff from 100 ms to a 30 s cap so a persistent dependency failure blocks ordering without hot-looping against that dependency. A failed purge whose marker *did* land remains repairable and keeps the existing log-and-continue behavior.
...
|---|---|---|
| `Account` → `b2b_org` | `org-settings` KV, configured teams, restored Account `ParentUID`, and the Salesforce child query | Accepted writers/auditors, team references, the org's parent, and its complete child set (`restoreOrgAccess`) |
| `Asset` → `project_membership` | Restored Asset fields, project resolver, and current Salesforce `Project_Role__c` records fetched once for the whole Asset batch | Structural `b2b_org`/`project` references plus one `member_put` per current key contact |

Direct principal restoration is additive: absent writers/auditors are excluded. Hierarchy and membership structural references are authoritative full-syncs because `delete_access` removed those tuples; an empty child set is a valid restored state.

Both `UNDELETE` and `GAP_UNDELETE` use this path. Current key contacts for all restored memberships are fetched in one chunked Salesforce batch and grouped by membership before any restore message is published; this avoids per-membership SOQL and prevents a source failure from producing a partial batch restore. Per-principal failures are isolated: one contact that cannot be republished does not prevent the remaining contacts from being attempted. Successful restore publishes are followed by one `Flush`; a batch fetch error or any authoritative-source read, transient resolver/LFID lookup, publish, current-grant index write, or flush failure retries the same event before cursor advancement. A definitive LFID `NotFound`, a genuinely empty principal/contact set, and structurally absent Salesforce associations advance because retry cannot manufacture missing source data; available references are still published. Failure to clear a `PendingRevoke` marker after its revoke was already flushed is logged but does not trigger retry; authorization already converged and the stale marker is bookkeeping debt.

Restore does not clear a delete-access failure marker: marker deletion is revision-conditional and the consumer has no per-key lookup, while markers may also predate the restore. Operators must therefore apply the fresh Salesforce state check in [Delete_access failure marker](#delete_access-failure-marker) before every manual replay.

An ID absent from a successful batch result advances as outside the service's managed projection: the Account query intentionally requires a live Membership Asset, and the Asset query intentionally requires `Product2.Family = 'Membership'`. Holding either absence would permanently stall the shared CDC channel for ordinary restores of non-qualifying Salesforce records. Deterministic conversion failures are likewise logged by the Salesforce reader and advance rather than replaying the same invalid data forever.

The consumer never falls back to `key-contact-grants`: that bucket addresses later revokes but can be incomplete for contacts granted before the index existed. The `membership` relation on `b2b_org` remains outside this service (`MembershipUIDs` is never populated).
...
Each upsert/delete evicts the record's sObject cache entry in the `member-service-cache` bucket (`CacheInvalidator.Invalidate*`, implemented by `salesforce.SObjectClient`). This keeps sObject-cache-backed readers — the HTTP API and the b2b_org reparenting `GetB2BOrg` lookup — from serving a stale copy. The CDC batch re-fetch itself is uncached SOQL, so it does not depend on this eviction. `InvalidateB2BOrg` evicts every B2BOrg key variant for the UID: `b2b_org` (legacy), `b2b_org_v2` / `b2b_org_v3` / `b2b_org_v3_noslug` (retired full-org projections), `b2b_org_v4` (current full-org), `b2b_org_flat`, and `b2b_org_parent_brief` — see `docs/agent-guidance/salesforce-cache.md` for the key inventory.

---

## Failure Modes & Log Signals

The consumer continues after ordinary repairable failures and uses `/admin/reindex`; authorization failures that reindex cannot repair retry the same event before consuming another.
...
| `cdc: b2b_org delete_access publish failed for uid …` / `cdc: project_membership delete_access publish failed for uid …` | A genuine-delete's `delete_access` publish failed; the UID was queued to `cdc-repair` under the delete-specific type (or logged separately if that write also failed — see below). | Follow the fail-closed Salesforce state check in [Delete_access failure marker](#delete_access-failure-marker) before any manual replay. Not repairable via `/admin/reindex`. |
| `cdc: failed to record delete_access failure for manual recovery — holding replay cursor` | The delete-specific repair marker write itself failed after all retries; the UID has no durable record at all. | Recover the UID from this log line, but apply the same fresh Salesforce state check before replaying. The cursor is held (next row). |
| `cdc: authorization change incomplete — retrying event before advancing` | A purge has no durable marker, or a restored access set was not completely rebuilt and confirmed. The consumer retries that event without consuming a later one. | Repair the dependency named by the accompanying error; retry resumes automatically. |
| `cdc: restored project_membership has no authoritative structural reference` | Salesforce lacks the membership's Account or project association, or the project slug is definitively unknown. Available references were restored, but the missing authorization edge cannot be rebuilt and the cursor advanced to avoid a permanent channel stall. | Correct the Salesforce association or onboard the project, then re-trigger an Asset update; the log includes `publish_failed_for_backfill_repair=true`. |
| `cdc: failed to read org settings for restored org — principals remain locked out` / `cdc: failed to batch-read current key_contacts for restored memberships — contacts remain locked out` | The **source** of the restore could not be read, so no grants were rebuilt (and none were withdrawn). | Fix the `org-settings` KV or Salesforce key-contact query dependency; the held event retries automatically while the process remains alive. If restart recovery is required, use the full entity-specific procedure under [Delete_access failure marker](#delete_access-failure-marker), not an ordinary update. |
| `key_contact FGA member_put publish failed` or a structural restore publish error | The source was read but a restore publish failed. Remaining independent publishes are still attempted. | Fix NATS delivery; the same event retries automatically. |
| `cdc: Salesforce API quota threshold reached: skipping delete sibling prefetch` | Quota pressure gated the `Project_Role__c` delete path's sibling recheck (see [Quota-gated key_contact delete recheck](#quota-gated-key_contact-delete-recheck)); every unsettled delete in the batch holds the replay cursor instead of advancing. | Wait for quota headroom to return (passive or active refresh); no manual repair needed, the held event retries automatically. |
| `fga_remove_raced_possible_lost_grant=true` | A reassert or repair `member_put` (or its flush) published to settle a race or a justified `PendingRevoke` marker failed. The caller's retry address (index entry or marker) is preserved. | Retried automatically on the next touch of that record; if persistent, check NATS delivery. |
| `fga_repair_unverified_possible_dangling_tuple=true` | The live re-verification read after a different-UID regrant race repair failed, so the repaired tuple's justification could not be confirmed. | Retried automatically; if persistent, check the sibling-lister dependency (Salesforce/NATS). |
...
| `LF_CONTRACTOR_TEAM_NAME` | Second LF team with the same blanket `auditor` grant (LFXV2-3071; chart sets `lf-contractor`; unset grants nothing) | `""` | No |
...
| `membership-cache` | Read/written by the injected `ProjectResolver` while resolving `project_uid` for `Asset` / `Project_Role__c` upserts — it looks up and populates `project-uid.<slug>` via `Storage.GetProjectUID` / `PutProjectUID`. The consumer also **evicts** `membership.<uid>` entries on every Asset upsert, delete, and absent-convergence (`DeleteMembership`, skipped when the evictor is nil) so `GetMemberTiers` does not keep serving pre-change tier/status, and **evicts** grouped `key-contacts.<membership_uid>` entries on every `Project_Role__c` upsert and delete (`DeleteKeyContactsForMembership`, also nil-skippable) so the member-tiers eligibility revalidation does not keep passing deactivated, reassigned, or deleted contacts; both read-through caches use revision-conditional write-backs that keep an in-flight fetch from undoing an eviction. | 6 h stale / 23 h expire |
