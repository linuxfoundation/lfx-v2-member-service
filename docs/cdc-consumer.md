...
| `Asset` → `project_membership` | Restored Asset fields, project resolver, and current Salesforce `Project_Role__c` records | Structural `b2b_org`/`project` references plus one `member_put` per current key contact |
