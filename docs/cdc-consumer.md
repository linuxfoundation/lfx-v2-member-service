...
| Delete | Invalidate cache → `PublishProjectMembershipIndexer` (`deleted`, stub — data is the UID string) → `delete_access` for the UID. Genuine-delete path only. On publish failure, same recovery story as `b2b_org` above, recorded under `ReindexTypeProjectMembershipDeleteAccess`. |
