...
| GET | `/b2b_orgs/member-tiers/{username}` | Highest active tier per org for a user (machine callers) | `member` on `team:{memberTiersCallerTeamName}` (defaults to `globalOrgAdminTeamName`) |
