...
// ProjectSlugAttribute is the shared DSL attribute for the caller-owned project
// slug on workspace project add/bulk-add bodies. Shared so the validation and
// description cannot drift between the single-add and bulk-item shapes.
func ProjectSlugAttribute() {
	dsl.Attribute("project_slug", dsl.String, "Caller-owned project identifier (for example, an Org Lens project slug). Idempotency key within the workspace; member-service generates the project_uid.", func() {
		dsl.MinLength(1)
		dsl.MaxLength(512)
		dsl.Example("kubernetes")
	})
...
// ProjectNameAttribute is the shared DSL attribute for the optional
// human-readable project name on workspace project add/bulk-add bodies.
func ProjectNameAttribute() {
	dsl.Attribute("project_name", dsl.String, "Optional human-readable project name for indexer search and display", func() {
		dsl.MaxLength(255)
		dsl.Example("Kubernetes")
	})
}
...
var AdminReindexPayload = dsl.Type("admin-reindex-payload", func() {
	...
	dsl.Attribute("items", dsl.ArrayOf(AdminReindexItem), "Targeted list of entity UIDs to reindex (surgical mode), all of the top-level type. Mutually exclusive with since, until, and cdc_repair. Max 100 items; when present, must be non-empty (an explicit empty array is rejected rather than silently falling through to a full reindex).", func() {
		dsl.MaxLength(100)
	})
...
var WorkspaceProjectResponse = dsl.Type("workspace-project-response", func() {
	...
	dsl.Attribute("project_uid", dsl.String, "Association UID — a UUID generated by member-service. Stable for indexing and is the delete key.", func() {
		dsl.Format(dsl.FormatUUID)
		dsl.Example("a1b2c3d4-e5f6-7890-abcd-ef1234567890")
	})
	dsl.Attribute("project_slug", dsl.String, "Caller-owned project identifier supplied on add (for example, an Org Lens project slug)", func() {
		dsl.Example("kubernetes")
	})
	dsl.Attribute("project_name", dsl.String, "Optional human-readable project name supplied on add", func() {
		dsl.Example("Kubernetes")
	})
	...
	dsl.Required("project_uid", "project_slug")
})
...
var WorkspaceProjectAddBody = dsl.Type("workspace-project-add-body", func() {
	...
	dsl.Required("project_slug")
})

// WorkspaceProjectAddItem is a single entry in a bulk workspace project add.
var WorkspaceProjectAddItem = dsl.Type("workspace-project-add-item", func() {
	dsl.Description("A single project to add to a workspace in a bulk request")
	ProjectSlugAttribute()
	ProjectNameAttribute()
	dsl.Required("project_slug")
})
...
var WorkspaceProjectsBulkAddBody = dsl.Type("workspace-projects-bulk-add-body", func() {
	...
	dsl.Attribute("projects", dsl.ArrayOf(WorkspaceProjectAddItem), "Projects to add; at most 100 per request", func() {
		dsl.MinLength(1)
		dsl.MaxLength(100)
	})
	dsl.Required("projects")
})
...
var WorkspaceBulkAddItemError = dsl.Type("workspace-bulk-add-item-error", func() {
	...
	dsl.Attribute("project_slug", dsl.String, "The project slug that failed", func() {
		dsl.Example("")
	})
	dsl.Attribute("error", dsl.String, "Reason the project could not be added", func() {
		dsl.Example("project_slug is required")
	})
	dsl.Required("project_slug", "error")
})
...
var WorkspaceBulkResponse = dsl.Type("workspace-bulk-response", func() {
	...
	dsl.Attribute("succeeded", dsl.ArrayOf(dsl.String), "Slugs of the projects that were successfully added (or were already present)", func() {
		dsl.Example([]string{"kubernetes"})
	})
