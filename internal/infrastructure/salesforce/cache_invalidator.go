...
// retired full-org projections (sobjectKeyPrefixB2BOrgV2Legacy,
// sobjectKeyPrefixB2BOrgV3Legacy, sobjectKeyPrefixB2BOrgV3NoSlugLegacy), the
// current full-org key (sobjectKeyPrefixB2BOrg), the flat-account fetch
// (sobjectKeyPrefixB2BOrgFlat), and the parent-brief fetch
// (sobjectKeyPrefixB2BOrgParentBrief). Any of the seven may exist across
// deploys, so callers invalidating an Account must not be left with a stale
// entry under any of them (see LFXV2-2654, lfx-self-serve#2570).
func (c *SObjectClient) InvalidateB2BOrg(ctx context.Context, uid string) error {
	return errors.Join(
		...
		c.InvalidateCache(ctx, sobjectCacheKey(sobjectKeyPrefixB2BOrgV3Legacy, uid)),
		c.InvalidateCache(ctx, sobjectCacheKey(sobjectKeyPrefixB2BOrgV3NoSlugLegacy, uid)),
