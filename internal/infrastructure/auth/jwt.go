...
const (
	...
	defaultAudience    = "lfx-v2-member-service"
...
// JWTAuth validates JWT tokens issued by Heimdall.
type JWTAuth struct {
...
// NewJWTAuth creates a new JWTAuth instance with the given configuration.
...
func (j *JWTAuth) ParsePrincipal(ctx context.Context, token string, logger *slog.Logger) (string, error) {
	...
	heimdallClaims, ok := claims.CustomClaims.(*HeimdallClaims)
	if !ok {
		// This should never happen.
	...
	logger.DebugContext(ctx, "JWT principal parsed",
		"principal", heimdallClaims.Principal,
		"email", heimdallClaims.Email,
	)

	return heimdallClaims.Principal, nil
}
