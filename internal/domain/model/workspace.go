...
// ProjectInfo carries resolved project identifiers. It is shared with the
// project resolver (which populates SFID); the workspace write path sets only
// UID, Slug, and Name.
...
// WorkspaceProject is a project association within a workspace.
type WorkspaceProject struct {
	// ProjectUID is the association's primary key — a UUID generated by
	// member-service. It is stable for indexing and is the delete key.
	ProjectUID string `json:"project_uid"`
	// ProjectSlug is the caller-owned project identifier and the idempotency
	// key within a workspace.
	ProjectSlug string `json:"project_slug"`
	// ProjectName is an optional human-readable name the caller supplies for
	// indexer search and display.
...
// WorkspaceProjectBySlug returns a pointer to the WorkspaceProject with the
// given project slug, or nil if not present. Slug is the per-workspace
// idempotency key for additions.
func (p *WorkspaceProjects) WorkspaceProjectBySlug(slug string) *WorkspaceProject {
	if p == nil {
		return nil
	}
	for i := range p.Projects {
		if p.Projects[i].ProjectSlug == slug {
			return &p.Projects[i]
		}
	}
	return nil
