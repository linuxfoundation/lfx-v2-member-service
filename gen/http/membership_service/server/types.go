...
type AdminReindexRequestBody struct {
	...
	CdcRepair *bool `form:"cdc_repair,omitempty" json:"cdc_repair,omitempty" xml:"cdc_repair,omitempty"`
...
type BulkAddB2bOrgWorkspaceProjectsRequestBody struct {
	...
	Projects []*WorkspaceProjectAddItemRequestBody `form:"projects,omitempty" json:"projects,omitempty" xml:"projects,omitempty"`
}
...
func NewBulkAddB2bOrgWorkspaceProjectsPayload(body *BulkAddB2bOrgWorkspaceProjectsRequestBody, uid string, workspaceUID string, version *string, bearerToken *string, ifMatch *string) *membershipservice.BulkAddB2bOrgWorkspaceProjectsPayload {
	...
	v.Projects = make([]*membershipservice.WorkspaceProjectAddItem, len(body.Projects))
	for i, val := range body.Projects {
		if val == nil {
			v.Projects[i] = nil
			continue
		}
		v.Projects[i] = unmarshalWorkspaceProjectAddItemRequestBodyToMembershipserviceWorkspaceProjectAddItem(val)
	}
	v.UID = uid
	v.WorkspaceUID = workspaceUID
	v.Version = version
...
func ValidateBulkAddB2bOrgWorkspaceProjectsRequestBody(body *BulkAddB2bOrgWorkspaceProjectsRequestBody) (err error) {
	...
	if len(body.Projects) < 1 {
		err = goa.MergeErrors(err, goa.InvalidLengthError("body.projects", body.Projects, len(body.Projects), 1, true))
	}
	if len(body.Projects) > 100 {
		err = goa.MergeErrors(err, goa.InvalidLengthError("body.projects", body.Projects, len(body.Projects), 100, false))
	}
	for _, e := range body.Projects {
		if e != nil {
			if err2 := ValidateWorkspaceProjectAddItemRequestBody(e); err2 != nil {
				err = goa.MergeErrors(err, err2)
			}
		}
	}
